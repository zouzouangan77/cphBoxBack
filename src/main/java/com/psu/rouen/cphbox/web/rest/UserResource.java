...
import com.psu.rouen.cphbox.web.rest.errors.EmailAlreadyUsedException;
import com.psu.rouen.cphbox.web.rest.errors.LoginAlreadyUsedException;
import java.net.URI;
 ...
 * <p>
 * This class accesses the {@link com.psu.rouen.cphbox.domain.User} entity, and needs to fetch its collection of authorities.
 * <p>
...
public class UserResource {
    ...
    public ResponseEntity<List<UserDTO>> getAllUserbySearch(@PathVariable String search, @org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        log.debug("REST request to get all User for an admin");
        if (!onlyContainsAllowedProperties(pageable)) {
            return ResponseEntity.badRequest().build();
        }

        final Page<UserDTO> page = userService.searchAllUser(search, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
